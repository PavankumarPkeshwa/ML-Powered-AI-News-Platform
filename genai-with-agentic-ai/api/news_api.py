...
# VectorDB ONLY for semantic search
from rag.vectordb import get_vector_db

router = APIRouter(prefix="/news", tags=["News"])

# Path to JSON cache (generated by GenAI every 6 hours)
CACHE_FILE = os.path.join("cache", "news_cache.json")


# --------------------------------------------------
# Utility: Read JSON cache safely
# --------------------------------------------------
def read_cache():
    if not os.path.exists(CACHE_FILE):
        return {"updatedAt": None, "articles": []}
...
# --------------------------------------------------
# FEATURED ARTICLE (FAST — JSON ONLY)
# --------------------------------------------------
@router.get("/featured")
def get_featured_article():
    try:
...
# --------------------------------------------------
# TRENDING ARTICLES (FAST — JSON ONLY)
# --------------------------------------------------
@router.get("/trending")
def get_trending_articles(limit: int = Query(3, ge=1, le=10)):
    try:
        ...
        trending = [a for a in articles if a.get("isTrending")]
...
# --------------------------------------------------
# SEMANTIC SEARCH (VECTOR DB — CORRECT USAGE)
# --------------------------------------------------
@router.get("/search")
def search_articles(q: str = Query(..., min_length=1)):
    try:
        ...
        for doc in docs:
            ...
            article = {
                ...
                "_id": article_id,
                "title": metadata.get("title", content[:100] + "..."),
                "excerpt": metadata.get("excerpt", content[:200] + "..."),
                "content": content,
                "imageUrl": metadata.get(
                    "imageUrl",
                    f"https://picsum.photos/seed/{article_id}/800/600"
                ),
                "author": metadata.get("author", "AI News Agent"),
                "publishDate": metadata.get(
                    "publishDate",
